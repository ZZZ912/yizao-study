...
export function ProfilePage({ user }: { user: User }) {
  ...
  return (
    <AppShell><main className="dashboard-shell narrow-content">
      ...
      <Card><h2>{user.display_name || "学习者"}</h2><p>{user.email}</p><p>目标：2026 一级造价工程师 · 土木建筑工程 · 湖北武汉</p><Button variant="secondary" type="button" onClick={() => logout.mutate()}>退出登录</Button></Card>
    </main></AppShell>
  );
}
